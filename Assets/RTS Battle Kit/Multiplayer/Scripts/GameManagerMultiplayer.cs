...
public class GameManagerMultiplayer : NetworkBehaviour {
	...
	void Update(){
		...
		if(GetComponent<CharacterManagerMultiplayer>().host){
			...
			if(player1CastleStrength <= 0 || player2CastleStrength <= 0){
				RpcEndBattleClientRpc(player1CastleStrength, player2CastleStrength, battleDuration, 
	...
	[ClientRpc]
	void RpcEndBattleClientRpc(float player1Castle, float player2Castle, float duration, int player1kills, int player2kills){
