...
public class CharacterManagerMultiplayer : NetworkBehaviour {
    ...
    void Update(){
        ...
        if(Input.GetMouseButtonDown(0)){
			...
			&& (!ownHalfOnly || (ownHalfOnly && ((transform.position.x < 0 && hit.point.x < -2) || (transform.position.x > 0 && hit.point.x > 2))))){
				...
				if(!host){
					SpawnUnitServerRpc(hit.point, selectedUnit);
				}
				else{
					...
					AssignTroop(newTroop, OwnerClientId);
					//tell the client to add tags locally
					SetRpcAddTagsClientRpc(newTroop.GetComponent<NetworkObject>().NetworkObjectId, "Player 1 unit", "Player 2 unit", "Player 2 castle", Color.red);
			...
			if(isPlacingBomb && !EventSystem.current.IsPointerOverGameObject()){
				...
				if(host){
					...
					SpawnExplosionServerRpc(hit.point);
				}
    ...
    public void AssignTroop(GameObject troop, ulong clientId)
    {
	    if (IsServer)
	    {
		    NetworkObject troopNetObj = troop.GetComponent<NetworkObject>();
		    troopNetObj.Spawn();
        
		    // Now assign ownership
		    troopNetObj.ChangeOwnership(clientId);
	    }
    }
	...
	[ServerRpc]
	void SpawnUnitServerRpc(Vector3 position, int unit){
		...
		AssignTroop(newTroop, OwnerClientId);
		SetRpcAddTagsClientRpc(newTroop.GetComponent<NetworkObject>().NetworkObjectId, "Player 2 unit", "Player 1 unit", "Player 1 castle", Color.blue);
		ParticleSystem effect = Instantiate(newUnitEffect, position, Quaternion.identity) as ParticleSystem;
		effect.GetComponent<NetworkObject>().Spawn();
	}
	...
	[ClientRpc]
    void SetRpcAddTagsClientRpc(ulong networkObjectId, string tag, string attackTag, string attackCastleTag, Color color){
	    if (NetworkManager.Singleton.SpawnManager.SpawnedObjects.TryGetValue(networkObjectId, out NetworkObject networkObject))
	    {
		    GameObject newTroop = networkObject.gameObject;
		    newTroop.tag = tag;

		    var character = newTroop.GetComponent<CharacterMultiplayer>();
		    if (character != null)
		    {
			    character.attackTag = attackTag;
			    character.attackCastleTag = attackCastleTag;
			    character.healthBarFill.GetComponent<Image>().color = color;
		    }
	    }
    }
	...
	public void enemyUnitKilled(string tag, string unitType){
		...
		if(!host){
			EnemyUnitKilledServerRpc(tag, unitType);
		}
		else{
			//if this is the host, directly send an rpc
			SetRpcEnemyUnitKilledClientRpc(tag, unitType);
		}
	}
	
	//server tells client to show the kill label
	[ServerRpc]
	void EnemyUnitKilledServerRpc(string tag, string unitType){
		SetRpcEnemyUnitKilledClientRpc(tag, unitType);
	}
	
	//client checks if it should show a label
	[ClientRpc]
    void SetRpcEnemyUnitKilledClientRpc(string tag, string unitType){
