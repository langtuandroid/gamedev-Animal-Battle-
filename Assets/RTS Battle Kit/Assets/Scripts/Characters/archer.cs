...
public class archer : MonoBehaviour {
	...
	void LateUpdate(){
		...
		if(addArrowForce && this.gameObject != null && GetComponent<Character>().currentTarget != null && newArrow != null && arrowSpawner != null){
			...
			newArrow.GetComponent<Rigidbody>().AddForce(transform.TransformDirection(new Vector3(0, shootingForce * 5 + 
			((GetComponent<Character>().currentTarget.transform.position.y - transform.position.y) * 45), shootingForce * 30)));
			addArrowForce = false;
		}
			...
			newArrow.GetComponent<Rigidbody>().AddForce(transform.TransformDirection(new Vector3(0, shootingForce * 5 + 
			((GetComponent<Character>().castleAttackPosition.y - transform.position.y) * 45), shootingForce * 20)));
			addArrowForce = false;
		}
	}
