...
public class Character : MonoBehaviour {
    ...
    void Start() {
        ...
        agent.stoppingDistance = castleStoppingDistance;
        ...
        mstats = this.GetComponent<StatsAB>();

        if (mstats)
        {
            LevelText1.text = LevelText2.text = "LEVEL "+ mstats.level;
            if (mstats.isPlayerR)
            {
                int spawnedCount = PlayerPrefs.GetInt("Spawned" + mstats.idD, 0);
                PlayerPrefs.SetInt("Spawned" + mstats.idD, spawnedCount + 1);
                PlayerPrefs.Save();
                LevelText1.text = " " + mstats.level;
            }
            lives = mstats.healthH;
            LevelText2.text = " " + mstats.level;
            damage = mstats.dmgG;
	...
	void Update(){
        ...
        if (levelObj)
            if (levelObj.activeSelf)
                levelObj.transform.LookAt(2 * transform.position - Camera.main.transform.position);

        if (Manager.canStart && !dead)
        {
            if (!st)
            {
                st = true;
    ...
    void CharacterLogic()
    {
        ...
        if (!goingToClickedPos)
        {
            ...
            if (currentTarget != null && Vector3.Distance(transform.position, currentTarget.transform.position) < minAttackDistance)
            {
                ...
                if ((Vector3.Distance(currentTarget.position, transform.position) > agent.stoppingDistance && !wizardSpawns) || Manager.victory || Manager.gameOver )
                {
                    foreach (Animator animator in animators)
                    {
        ...
        if (lives < 1 )
        {
            ...
            {
                int killedCount = PlayerPrefs.GetInt("Killed" + mstats.idD, 0);
                PlayerPrefs.SetInt("Killed" + mstats.idD, killedCount + 1);
                PlayerPrefs.Save();
                Manager.enemiesKilled++;
            }
            else
            {
                Manager.playersKilled++;
            }
            dead = true;
            health.SetActive(false);
    ...
    foreach(GameObject potentialCastle in castles){
        ...
        if(potentialCastle.GetComponent<Castle>().lives>0)
        if (Vector3.Distance(transform.position, potentialCastle.transform.position) < closestCastle && potentialCastle != null && potentialCastle.tag == attackCastleTag){
	...
	//Define a position to attack the castles(to spread characters when they are attacking the castle)
	        if(castle != null && castle.GetComponent<Castle>().lives > 0){
	...
	float closestDistance = Mathf.Infinity;
        ...
        else if (enemies.Length == 1 && !enemies[0].GetComponent<Character>().dead)
        {
            closestDistance = Vector3.Distance(transform.position, enemies[0].transform.position);
